...
static volatile int64_t curr_total_size_bytes = 0;
double ffmpeg_get_total_size_bytes(void);
double ffmpeg_get_total_size_bytes()
{
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time, int64_t pts)
{
    ...
    if (total_size > 0) {
        curr_total_size_bytes = (double) total_size;
    }
    if (pts == AV_NOPTS_VALUE) {
